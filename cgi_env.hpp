...
- SERVER_PROTOCOL 		: Le nom et la révision du protocole dans lequel la requête a été faite (Format : protocole/révision)
...
- PATH_TRANSLATED		: Contient le chemin demandé par le client après que les conversions virtuel → physique ont été faites par le serveur.
...
- HTTP_ACCEPT_LANGUAGE 	: Les langues dans lequel le client accepte de recevoir la réponse.
- HTTP_USER_AGENT 		: Le navigateur utilisé par le client.
- HTTP_COOKIE 			: Les éventuels cookies. Une liste de paires clef=valeur contenant les cookies positionnés par le site, séparés par des points-virgules.
- HTTP_REFERER 			: Une adresse absolue ou partielle de la page web à partir de laquelle la requête vers la page courante a été émise.
